...
int main(int argc, char** argv){
    ...
    }else{          // 若dirDest!=NULL 则 dest 已存在，文件夹复制失败
...
void MyCopy(const char *src, const char *dest){
    ...
    while ((entry = readdir(dir))!=NULL){   // 获取一个子文件/文件夹
        ...
        // 路径名拼接 基路径/文件名（文件夹名）
		strcpy(child_dest_path, dest);            // 赋值子文件/文件夹的基路径
		strcpy(child_src_path, src);
		strcat(child_src_path, "/");              // 加上文件连接符 /
		strcat(child_dest_path, "/");
		strcat(child_dest_path, entry->d_name);   // 加上文件名(文件夹名)
...
void copyFile(const char *src, const char *dest){
    ...
    // 新建一个文件，并得到该文件的描述符
	if((srcFd = creat(dest, filestat.st_mode)) < 0){ 
        printf("create file : %s error.\n", dest);
        return ;
    };
    // 打开源文件的，并得到源文件描述符
	if ((destFd = open(src, O_RDONLY)) < 0) {
